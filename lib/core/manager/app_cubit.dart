...
// coins ***********
  Future addAdCoins() async
  {
    ...
    await CacheHelper.saveData(key: CacheHelperKeys.coinsKey, value:  CacheHelperKeys.coinsNumber);
...
// END coins ***********
  ...
  void deleteQuestionData()
  {
    ...
    suggest = false;
    remove = false;
    for (CharModel char in collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters )
    {
      char.isChosen = false;
    }
  ...
  void sampleOnTap (int index) async
  {
    if(value.isEmpty)
    {
        value.add(collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[index]);
        collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[index].isChosen = true;
  ...
  void whereToAdd(int index)
  {
    for (int i = 0; i < value.length; i++){
      if(value[i] == null){
        value[i] = collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[index];
        collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[index].isChosen = true;
        emit(SampleOnTapState());
        break;
      }
      else if( i+1 == value.length &&value.length !=collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].answer.length)
      {
        value.add(collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[index]);
        collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[index].isChosen = true;
        emit(SampleOnTapState());
  ...
  {
    if(value.length ==collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].answer.length)
    {
      String answerText="";
      bool check=true;
      for (int i = 0; i < value.length; i++){
      ...
      if(check) {
        if (answerText == collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].answer) {
          answered = true;
  ...
  Future levelUp() async
  {
    ...
    if(CacheHelperKeys.questionIndex! + 1 == collections[CacheHelperKeys.collectionIndex!].questions.length )
    {
      ...
      else
      {
        ...
        await CacheHelper.saveData(key: CacheHelperKeys.collectionIndexKey, value: CacheHelperKeys.collectionIndex);
        CacheHelperKeys.questionIndex = 0;
        await CacheHelper.saveData(key: CacheHelperKeys.questionIndexKey, value: CacheHelperKeys.questionIndex);
        CacheHelperKeys.solvedNumber = 0;
        await CacheHelper.saveData(key: CacheHelperKeys.solvedKey, value: CacheHelperKeys.solvedNumber);
        emit(LevelDoneState());
      }
    }
    else
  ...
  void answerOnTab(int index)
  {
      ...
      collections[CacheHelperKeys.collectionIndex!].questions[CacheHelperKeys.questionIndex!].characters[value[index]!.id].isChosen = false;
      value[index] = null;
      answered = null;
  ...
  void removeTwo() async
  {
    if (CacheHelperKeys.coinsNumber! < 5)
    {
      ...
      await deleteCoins();
      for (int j = 0; j < 2; j++) {
        for (int i = 0; i < collections[CacheHelperKeys.collectionIndex!]
            .questions[CacheHelperKeys.questionIndex!].characters.length; i++) {
          if (!collections[CacheHelperKeys.collectionIndex!]
              .questions[CacheHelperKeys.questionIndex!].answer.
          contains(collections[CacheHelperKeys.collectionIndex!]
              .questions[CacheHelperKeys.questionIndex!].characters[i].char)
              && collections[CacheHelperKeys.collectionIndex!]
                  .questions[CacheHelperKeys.questionIndex!].characters[i]
                  .isChosen == false) {
            collections[CacheHelperKeys.collectionIndex!]
                .questions[CacheHelperKeys.questionIndex!].characters[i]
                .isChosen = true;
  ...
  void suggestOne()async {
    if (CacheHelperKeys.coinsNumber! < 5) {
      ...
      for (int i = 0; i < collections[CacheHelperKeys.collectionIndex!]
          .questions[CacheHelperKeys.questionIndex!].characters.length; i++) {
        if (collections[CacheHelperKeys.collectionIndex!]
            .questions[CacheHelperKeys.questionIndex!].answer[0] ==
            collections[CacheHelperKeys.collectionIndex!]
                .questions[CacheHelperKeys.questionIndex!].characters[i].char) {
          if (value.isEmpty) {
  ...
  [
    ...
    CollectionModel(
        ...
        [
          ...
          QuestionModel(
              text: 'سمعني الكثير، ولكن لم يراني أحد، ولن أتكلم مرة أخرى ما لم تتكلم',
              answer: 'الصدي',
              characters: [
                CharModel(id: 0, char: 'ح'),
                CharModel(id: 1, char: 'ب'),
